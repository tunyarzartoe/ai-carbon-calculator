/* =========================================================
   CO2環境学習 40問ガイド スクリプト
   IIFEでカプセル化し、グローバル変数の衝突を防止。
   window.openQuestionChat / window.closeQuestionChat を公開。
   ========================================================= */
(function () {
    ...
    function appendMessage(text, isUser = false) {
        const chatBox = document.getElementById("questionChatBox");
        if (!chatBox) return;

        const rowDiv = document.createElement("div");
        rowDiv.className = `question-message-row ${isUser ? 'user' : 'ai'}`;

        const contentWrapper = document.createElement("div");
        contentWrapper.className = "question-message-content-wrapper";

        const bubble = document.createElement("div");
        bubble.className = "question-message-bubble";

        if (!isUser) {
            const label = document.createElement("div");
            label.className = "question-speaker-label-inside";
        ...
        meta.className = "question-message-meta";

        if (isUser) {
            const readSpan = document.createElement("span");
            readSpan.className = "question-read-status";
    ...
    function showTyping() {
        ...
        el.className = "question-typing";
        el.id = "questionTypingIndicator";
    ...
    function removeTyping() {
        const el = document.getElementById("questionTypingIndicator");
        if (el) el.remove();
    }

    function toggleModal(isOpen) {
        const modal = document.getElementById('questionModalMenu');
        if (modal) {
            modal.hidden = !isOpen;
        }
    }
    ...
    function renderTabs() {
        const container = document.getElementById("questionTabContainer");
        if (!container) return;
        container.innerHTML = "";
        categories.forEach((catName, idx) => {
            const btn = document.createElement("button");
            btn.type = "button";
            btn.className = `question-tab-btn ${idx === currentTab ? 'active' : ''}`;
    ...
    function initQuestionChat() {
        if (isInitialized) return;
        isInitialized = true;

        const closeBtn = document.getElementById('closeQuestionChatBtn');
        ...
        if (closeBtn) closeBtn.onclick = closeQuestionChat;
    ...
    function openQuestionChat() {
        ...
        if (typeof window.switchPage === 'function') {
            window.switchPage('questionPage');
        } else {
            document.querySelectorAll('.app-page').forEach(p => p.classList.toggle('active', p.id === 'questionPage'));
        }
        if (window.setActiveTab) window.setActiveTab('navChat');
        const chatBox = document.getElementById("questionChatBox");
    ...
    // グローバル衝突を避けつつ、外部から開閉できるように安全に関数を公開
    window.openQuestionChat = openQuestionChat;
    window.closeQuestionChat = closeQuestionChat;

    // URLパラメータ(?open=questions)またはハッシュ(#questions)で直接開かれた場合の処理
    function checkUrlTrigger() {
        try {
            const params = new URLSearchParams(window.location.search);
            if (params.get('open') === 'questions' || window.location.hash === '#questions') {
                openQuestionChat();
            }
        } catch (e) {
            /* ignore */
